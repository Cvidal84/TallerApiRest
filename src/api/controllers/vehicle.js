...
const getVehicleByPlate = async (req, res, next) => {
    try {
        ...
        if(!plate) {
            return res.status(400).json({ error: "Debes enviar una matrícula que esté registrada" });
        }
...
const getVehicleByClientId = async (req, res, next) => {
    try {
        ...
        if (!clientId) {
            return res.status(400).json({error: "Debes enviar un cliente"});
        }

        const vehicles = await Vehicle.find({ clientId });

        if (vehicles.length === 0) {
            return res.status(400).json({error: "Este cliente no tiene vehículos registrados"});
        }

        return res.status(200).json(vehicles);
        
    } catch (error) {
        return res.status(500).json({ error: "Error al buscar vehículos del cliente" });
    }
}
...
const postVehicle = async (req, res, next) => {
    try {
        ...
        return res.status(500).json({ error: "Error al crear el vehículo", details: error.message });
